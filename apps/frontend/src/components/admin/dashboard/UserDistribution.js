...
const UserDistribution = ({ data }) => {
  const [chartData, setData] = useState(data || { genders: [], roles: [] });
  ...
  if (!chartData.genders || !chartData.roles) {
    return <div>Error: Invalid data structure</div>;
  }

  // Calculate totals for percentage calculations
  const genderTotal = chartData.genders.reduce(
    (sum, item) => sum + item.value,
    0,
  );
  const roleTotal = chartData.roles.reduce((sum, item) => sum + item.value, 0);

  // Add total to each item for tooltip percentage calculation
  const genderData = chartData.genders.map((item) => ({
    ...item,
    total: genderTotal,
  }));

  const roleData = chartData.roles.map((item) => ({
    ...item,
    total: roleTotal,
  }));
  ...
  const ChartSection = ({ title, data, colors }) => (
    <div className="w-1/2 h-full">
      <h3
        className={`${isMobile ? "text-sm" : "text-sm"} font-semibold text-center mb-2 text-gray-300`}
      >
        {title}
      </h3>
  ...
  return (
    <div className="w-full h-full flex">
      ...
      />
      <ChartSection title="User Roles" data={roleData} colors={ROLE_COLORS} />
    </div>
  );
};
