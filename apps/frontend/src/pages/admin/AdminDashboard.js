...
const DashboardOverview = () => {
  ...
  const [userSummaryData, setSummaryData] = useState([]);
  const [userDistributionData, setUserDistributionData] = useState([]);
  const [recentActivityData, setRecentActivityData] = useState([]);
  const [userActivityOverview, setUserActivityOverview] = useState([]);
  const [userEngagementOverview, setUserEngagementOverview] = useState([]);
  ...
  const fetchData = async () => {
    ...
    try {
      const [
        ...
        MetricsAPI.getMetricsDistributions(),
        MetricsAPI.getMetricsRecentActivities(),
        MetricsAPI.getMetricsActivityOverview(),
        MetricsAPI.getMetricsEngagementOverview(),
      ]);

      setSummaryData(summaryData || []);
      setUserDistributionData(userDistributionData || []);
      setRecentActivityData(recentActivityData || []);
      setUserActivityOverview(userActivityOverview || []);
      setUserEngagementOverview(userEngagementOverview || []);
    } catch (error) {
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        <Card className="bg-black border border-gray-800 rounded-xl shadow-lg overflow-hidden">
          <CardHeader className="border-b border-gray-800 p-3">
            <h2 className="text-lg font-semibold text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-600">
              Activity Overview
            </h2>
          </CardHeader>
          <CardBody className="p-3" style={{ height: "250px" }}>
            <ActivityOverview data={userActivityOverview} />
          </CardBody>
        </Card>
        <Card className="bg-black border border-gray-800 rounded-xl shadow-lg overflow-hidden">
          <CardHeader className="border-b border-gray-800 p-3">
            <h2 className="text-lg font-semibold text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-blue-600">
              Engagement Trend
            </h2>
          </CardHeader>
          <CardBody className="p-3" style={{ height: "250px" }}>
            <EngagementTrend data={userEngagementOverview} />
          </CardBody>
        </Card>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
        <Card className="bg-black border border-gray-800 rounded-xl shadow-lg overflow-hidden lg:col-span-2">
          <CardHeader className="border-b border-gray-800 p-3">
            <h2 className="text-lg font-semibold text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-red-600">
              User Distribution
            </h2>
          </CardHeader>
          <CardBody className="p-3" style={{ height: "250px" }}>
        ...
        <Card className="bg-black border border-gray-800 rounded-xl shadow-lg overflow-hidden">
          <CardHeader className="border-b border-gray-800 p-3">
            <h2 className="text-lg font-semibold text-transparent bg-clip-text bg-gradient-to-r from-pink-400 to-red-600">
              Recent Activity
            </h2>
          </CardHeader>
